...
* У учителя есть темы  
* У учителя есть слоты(freetime), когда он может принять урок. Слоты не пересекаются между собой у одного учителя.
* У слота есть уроки, которые могут встать только в период слота.
* У урока есть тема, которая есть у учителя. Уроки не пересекаются между собой в слоте.